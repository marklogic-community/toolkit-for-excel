...
try{
  ...
  let $cpf-path := fn:concat($config:SERVER-ROOT,"Modules/MarkLogic/conversion/actions/")
  let $install-path := fn:concat($config:SERVER-ROOT,"Installer/openxml/")
  ...
  let $dest-support-1 := fn:concat($openxml-path, $support-file-1)
  let $dest-support-2 := fn:concat($openxml-path, $support-file-2)
  ...
  return (xdmp:save($dest-support-1, $support-doc-1),
          ...
          fn:concat("Step 1: install-xqy-support(); ",$support-file-1," copied to ",$openxml-path),
          fn:concat("Step 1: install-xqy-support(); ",$support-file-2," copied to ",$openxml-path),
          fn:concat("Step 1: install-xqy-support(); ",$cpf-action-1," copied to ",$cpf-path),
          fn:concat("Step 1: install-xqy-support(); ",$cpf-action-2," copied to ",$cpf-path),
          fn:concat("Step 1: install-xqy-support(); ",$cpf-config-1," copied to ",$install-path),
          fn:concat("Step 1: install-xqy-support(); ",$cpf-config-2," copied to ",$install-path)
          )
		  ...
		  at '/MarkLogic/cpf/domains.xqy';
           ...
           declare variable $config:cpf-path as xs:string external;
              ...
              (p:insert(xdmp:document-get('Installer/cpf/status-pipeline.xml')),
               ...
               (
                 (xs:QName("config:cpf-path"), fn:concat($config:SERVER-ROOT,"Installer/openxml/"))),
               <options xmlns="xdmp:eval">
		    <database>{xdmp:database($config:TRIGGERS-DB)}</database>
               </options>),
