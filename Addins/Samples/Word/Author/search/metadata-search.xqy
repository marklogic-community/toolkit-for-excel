 ...
 as element(ps:results-page)
{
  ...
  let $results := cts:search(//dc:metadata,$query)[ $start to $stop ]
  return
...
declare function ps:get-query($raw as xs:string)
 as cts:query?
{
...
return	xdmp:quote(
  	    ...
  	    let $res := <div>
                        {
                         if(fn:not($hits)) then
	                       (<div id="searchresultsinner"><p>Your search for "{$q}" did not match anything.</p></div>)
		         else
                               (   
                                ...
                                <strong>Results:</strong>
                                {
                                    ...
                                    return if($page gt 10) then
                                             (<a href="javascript:compareSearchAction({$page - 10});" class="leftpagination"> <img src="images/arrow-small-left.png"  border="0"/></a>,
                                                 $span,
                                              <a href="javascript:compareSearchAction({$new-page});" class="rightpagination"> <img src="images/arrow-small-right.png"  border="0"/></a>)
                                           else 
                                              ("&nbsp;&nbsp;",$span,<a href="javascript:comparSearchAction({$new-page});" class="rightpagination"> <img src="images/arrow-small-right.png"  border="0"/></a>)
                                                                   
                                     else if($new-start gt 10) then
                                             (<a href="javascript:compareSearchAction({$new-start - 10});" class="leftpagination"> <img src="images/arrow-small-left.png"  border="0"/></a>,
                                              $span
                                             )
                                     else  ("&nbsp;","&nbsp;",$span)
                                } 
				...
				let $uri := fn:data($hit/@uri)
                                ...
                                let $doc-folder := fn:substring-before($hit/@uri, "/customXml/item")
				...
				return 
					 ...
					 <div class="mergeactions">
                                            <a href="javascript:mergeDocuments({fn:concat($doc-folder,"/")});" onmouseup="blurSelected(this);" class="smallbtn mergebtn">Compare</a>
                                         </div>
                                 </div>
                               ) } </div>
           return ($res)
