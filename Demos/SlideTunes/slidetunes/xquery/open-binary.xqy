...
(: let $docname := "/out/myPlaylist.pptx" :) 
let $docname :=  xdmp:url-decode(xdmp:get-request-field("uri") )

let $log := xdmp:log(fn:concat("==================================DOCNAME: ",$docname))
(: let $title := xdmp:get-request-field("title") :)
let $title := fn:tokenize($docname,"/")[last()]
