...
package: 
	...
	cp Addins/Word/README.txt $(PUB_BUILD)
	cp LICENSE.txt $(PUB_BUILD)
	cp Addins/Word/NOTICE.txt $(PUB_BUILD)
	#begin 323
	...
	cp $(JS)/MarkLogicWordAddin.js $(BUILD_JS)/MarkLogicWordAddin.js
	cp $(JS)/MarkLogicContentControlSupport.js $(BUILD_JS)/MarkLogicContentControlSupport.js
	...
	cp $(DOCS)/ToolkitForWordGuide.docx $(BUILD_DOCS)
